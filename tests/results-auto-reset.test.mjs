...
test('5G, x2 and SIM spawn rates increase independently while collectible speeds stay unchanged', async () => {
  const game = await read('src/components/GameExperienceImpl.jsx')
  ...
  assert.match(game, /SIM_SPAWN_RATE_MULTIPLIER = 1\.55736/)
  assert.match(game, /NETWORK_SPAWN_WEIGHT = 0\.0735/)
  assert.match(game, /FIVE_G_SPAWN_WEIGHT = 0\.121/)
