...
test('September Sky feedback remains implemented across lock, home, game and video call', async () => {
  ...
  assert.doesNotMatch(home, /phone-statusbar|iphone-status-time/)
  ...
  assert.match(game, /PUNTEGGIO/)
  assert.match(game, /Con Sky Mobile puoi avere anche giga illimitati/)
  ...
  assert.match(video, /Scorri per rispondere/)
  assert.match(video, /📞/)
