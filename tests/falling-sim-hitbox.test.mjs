...
test('falling SIMs use a 105 percent hitbox without changing the visible chip or game2', async () => {
  const [game, game2, css] = await Promise.all([
  ...
  assert.match(css, /\.collectible--hitbox-105::before\s*\{[\s\S]*?inset:\s*-2\.5%[\s\S]*?pointer-events:\s*auto[\s\S]*?\}/)
  assert.match(css, /\.collectible--sim \.falling-chip\s*\{[\s\S]*?width:\s*92%[\s\S]*?height:\s*92%/)
})
