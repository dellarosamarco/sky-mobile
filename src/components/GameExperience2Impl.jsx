...
const COLLECTIBLE_LIFETIME_MS = 2000
...
export default function GameExperience2({ onReset }) {
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      setCollectibles((current) => current.filter((item) => now - item.spawnedAt < COLLECTIBLE_LIFETIME_MS))
  ...
  return (
    <main className={`game-field tap-game ${boostType ? `is-boosting boost-${boostType}` : ''}`} onPointerDown={() => audio.ensureContext()}>
      ...
      {collectibles.map((item) => (
        <button
          key={item.id}
          className={`collectible game2-collectible collectible--${item.type}`}
          type="button"
